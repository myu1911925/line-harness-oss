...
ページ上部に直近7日間の増減テーブルを表示。右上に「現在の友だち総数」を合わせて表示する。

- **データ保存期間**: 全期間分が `friend_events` テーブルに記録されている。表示が7日間なのは表示フィルターのみで、過去データは消えない。
- **増減の定義**: 追加（follow イベント数）・ブロック（unfollow イベント数）・純増（追加 − ブロック）。
