...
管理画面 `/auto-replies` から GUI で自動応答を管理できる。

| 操作 | 内容 |
|------|------|
| 一覧表示 | キーワード・マッチ方法・応答タイプ・有効/無効を一覧表示 |
| 新規作成 | 「＋ 新規追加」ボタンからフォーム入力 |
| 編集 | 各行の「編集」ボタンからフォームを開いて更新 |
| 有効/無効切替 | 各行のバッジをクリックで即時切替 |
| 削除 | 各行の「削除」ボタン（確認ダイアログあり） |

**ポストバック（リッチメニューボタン）との連携:**  
リッチメニューのボタンに `postback` アクションを設定し、`data` フィールドの値（例: `action_id=17658825`）をキーワードとして登録することで、ボタンタップ時に自動返信できる。マッチタイプは `contains`（部分一致）を推奨。
